...
**Files:**
- Create: `slides/module-1-power-bi.md`
- Create: `slides/README.md`
- Create: `slides/build/module-1-power-bi.pdf` (rendered output, committed)

**Acceptance Criteria:**
- [ ] `slides/module-1-power-bi.md` has Marp front-matter (`marp: true`, `theme: gaia`, `paginate: true`, footer) and ~8–9 slides matching the spec breakdown.
- [ ] `npx @marp-team/marp-cli slides/module-1-power-bi.md --pdf -o slides/build/module-1-power-bi.pdf` produces a PDF with no error.
- [ ] `slides/README.md` documents the render commands (PDF/PPTX/HTML) and the theme choice.

**Verify:** `npx -y @marp-team/marp-cli slides/module-1-power-bi.md --pdf -o slides/build/module-1-power-bi.pdf && test -f slides/build/module-1-power-bi.pdf && echo OK` → prints `OK`

**Steps:**

- [ ] **Step 1: Write `slides/module-1-power-bi.md`**
...
Run: `npx -y @marp-team/marp-cli slides/module-1-power-bi.md --pdf -o slides/build/module-1-power-bi.pdf`
Expected: command exits 0; `slides/build/module-1-power-bi.pdf` created. (First run downloads Marp + a headless Chromium; allow time. If the network blocks the download, skip PDF generation, note it, and commit the `.md` source only — the PDF can be rendered later with the same command.)
...
| Fail | Slot |
|------|------|
| `module-1-power-bi.md` | Slot 1 — Power BI (~20 min) |
...
```bash
git add slides/module-1-power-bi.md slides/README.md slides/build/module-1-power-bi.pdf
...
**Files:**
- Create: `slides/module-2-notebooklm-gemini.md`
- Create: `slides/build/module-2-notebooklm-gemini.pdf` (rendered output, committed)

**Acceptance Criteria:**
- [ ] `slides/module-2-notebooklm-gemini.md` has matching Marp front-matter and ~8 slides per the spec breakdown, including the data-sensitivity warning slide.
- [ ] PDF renders without error.

**Verify:** `npx -y @marp-team/marp-cli slides/module-2-notebooklm-gemini.md --pdf -o slides/build/module-2-notebooklm-gemini.pdf && test -f slides/build/module-2-notebooklm-gemini.pdf && echo OK` → prints `OK`

**Steps:**

- [ ] **Step 1: Write `slides/module-2-notebooklm-gemini.md`**
...
Run: `npx -y @marp-team/marp-cli slides/module-2-notebooklm-gemini.md --pdf -o slides/build/module-2-notebooklm-gemini.pdf`
Expected: command exits 0; PDF created. (Same network caveat as Task 1 Step 2.)

- [ ] **Step 3: Commit**

```bash
git add slides/module-2-notebooklm-gemini.md slides/build/module-2-notebooklm-gemini.pdf
...
**Files:**
- Create: `module-1-power-bi/lab/img/.gitkeep`
- Modify: `module-1-power-bi/lab/01-import-data.md` (normalize `../img/` → `img/`; add result shot)
- Modify: `module-1-power-bi/lab/02-power-query-bersih.md` (add 2 placeholders)
- Modify: `module-1-power-bi/lab/03-model-hubungan.md` (normalize `../img/` → `img/`; add cardinality shot)
- Modify: `module-1-power-bi/lab/04-ukuran-dax.md` (add 1 placeholder)
- Modify: `module-1-power-bi/lab/05-visual-asas.md` (normalize `../img/` → `img/`; add card shot)
- Modify: `module-1-power-bi/lab/06-dashboard-interaktif.md` (normalize `../img/` → `img/`; add slicer shot)
- Modify: `module-1-power-bi/lab/07-eksport-simpan.md` (add 1 placeholder)

**Acceptance Criteria:**
- [ ] `module-1-power-bi/lab/img/` exists (via `.gitkeep`).
...
**Verify:** `! grep -rn "\.\./img/" module-1-power-bi/lab/ && grep -rEc "\!\[.*\]\(img/" module-1-power-bi/lab/*.md` → no `../img/` matches; each lab reports its placeholder count.
...
```bash
mkdir -p module-1-power-bi/lab/img
printf '' > module-1-power-bi/lab/img/.gitkeep
```

- [ ] **Step 2: Lab 01 — normalize existing ref + add result shot**

In `module-1-power-bi/lab/01-import-data.md`, change the existing line `   ![Get Data Text/CSV](../img/01-get-data.png)` to `   ![Get Data Text/CSV](img/01-get-data.png)`.
...
In `module-1-power-bi/lab/02-power-query-bersih.md`, after step 3 (the data-type list ending `… → **Text**`), insert:
...
In `module-1-power-bi/lab/03-model-hubungan.md`, change `   ![Model view star schema](../img/03-model.png)` to `   ![Model view star schema](img/03-model.png)`.
...
In `module-1-power-bi/lab/04-ukuran-dax.md`, after the "Cara cipta measure" paragraph ending `… tekan **Enter**.`, insert:
...
In `module-1-power-bi/lab/05-visual-asas.md`, change `![Visual asas](../img/05-visual.png)` to `![Visual asas](img/05-visual.png)`.
...
In `module-1-power-bi/lab/06-dashboard-interaktif.md`, change `   ![Dashboard interaktif](../img/06-dashboard.png)` to `   ![Dashboard interaktif](img/06-dashboard.png)`.
...
In `module-1-power-bi/lab/07-eksport-simpan.md`, in step 2, after the line `   - Power BI menjana PDF setiap halaman laporan untuk dikongsi atau dicetak.`, insert:
...
```bash
grep -rn "\.\./img/" module-1-power-bi/lab/   # expect: no output
git add module-1-power-bi/lab/
...
**Files:**
- Create: `module-2-notebooklm-gemini/lab/img/.gitkeep`
- Modify: `module-2-notebooklm-gemini/lab/01-set-up-notebook.md` (2 placeholders)
- Modify: `module-2-notebooklm-gemini/lab/02-query-the-corpus.md` (1 placeholder)
- Modify: `module-2-notebooklm-gemini/lab/03-project-paper-to-slides.md` (2 placeholders)
- Modify: `module-2-notebooklm-gemini/lab/04-meeting-audio-to-minutes.md` (1 placeholder)
- Modify: `module-2-notebooklm-gemini/lab/05-explore-further.md` (1 placeholder)

**Acceptance Criteria:**
- [ ] `module-2-notebooklm-gemini/lab/img/` exists (via `.gitkeep`).
- [ ] Each of the 5 labs has 1–2 placeholders matching `![...](img/NN-...png)` at the steps below.
- [ ] No `../img/` references introduced.

**Verify:** `grep -rEc "\!\[.*\]\(img/" module-2-notebooklm-gemini/lab/*.md && ! grep -rn "\.\./img/" module-2-notebooklm-gemini/lab/` → each lab reports ≥1 placeholder; no `../img/` matches.
...
```bash
mkdir -p module-2-notebooklm-gemini/lab/img
printf '' > module-2-notebooklm-gemini/lab/img/.gitkeep
```

- [ ] **Step 2: Lab 01 — 2 placeholders**

In `module-2-notebooklm-gemini/lab/01-set-up-notebook.md`, after step 3 (`3. Klik **Add sources** (Tambah sumber).`), insert:
...
In `module-2-notebooklm-gemini/lab/02-query-the-corpus.md`, after step 4 (ending `… sahkan jawapan itu benar-benar daripada dokumen.`), insert:
...
In `module-2-notebooklm-gemini/lab/03-project-paper-to-slides.md`, after Bahagian A step 3 (the briefing/study-guide list ending `… pecahan topik utama dan soalan.`), insert:
...
In `module-2-notebooklm-gemini/lab/04-meeting-audio-to-minutes.md`, after step 4 (the meeting-minutes table prompt code block), insert:
...
In `module-2-notebooklm-gemini/lab/05-explore-further.md`, after the blockquote `> Cuba jana **Audio Overview** … secara perbualan.`, insert:
...
```bash
grep -rn "\.\./img/" module-2-notebooklm-gemini/lab/   # expect: no output
git add module-2-notebooklm-gemini/lab/
...
**Verify:** `for f in $(grep -rhoE "img/[0-9]+-[a-z-]+\.png" module-1-power-bi/lab module-2-notebooklm-gemini/lab | sort -u); do grep -q "$(basename $f)" docs/SCREENSHOTS.md || echo "MISSING: $f"; done` → no `MISSING:` lines.
...
## Modul 1 — Power BI (`module-1-power-bi/lab/img/`)
...
Run: `for f in $(grep -rhoE "img/[0-9]+-[a-z-]+\.png" module-1-power-bi/lab module-2-notebooklm-gemini/lab | sort -u); do grep -q "$(basename $f)" docs/SCREENSHOTS.md || echo "MISSING: $f"; done`
